...
public class SaveLoadUIState : MonoBehaviour, IUIContollerState
{
    ...
    private UIController.STATE myState;
    ...
    public void StartUI()
    {
        ...
        if (UIController.Instance.CurrentState == UIController.STATE.SAVE)
        {
            confirmText.text = "Save Data?";
            labelText.text = "SAVE";
            confirmButton.onClick.AddListener(SaveAtSelectedSlot);
            myState = UIController.STATE.SAVE;
        }
        else
        {
            ...
            myState = UIController.STATE.LOAD;
        }
    ...
    private void LoadSavedData()
    {
        ...
        SceneController.Instance.LoadStage();
    ...
    private void SetDataSlots()
    {
        ...
        if (IUIContollerState.isConsole)
        {
            if(UIController.Instance.CurrentState == UIController.STATE.SAVE && currentPage == 1)
            {
    ...
    class DataSlot
    {
        ...
        {
            if (slotData == null && UIController.Instance.CurrentState == UIController.STATE.LOAD)
                return;
